...
<meta charset="utf-8">
    ...
    <title>Custer wise order | amazon</title>
...
<header id="header"><!--header-->
	 ...
	 <div id="contact-page" class="container">
    	<div class="bg">
	    	<div class="row">    		
	    		<div class="col-sm-12">    			   			
					<h2 class="title text-center">Customer wise order</h2>    			 
				</div>			 		
			</div>
...
<form>
				...
				<p>Select company name:<br>
				<select name="cid">
				<%
				while(rs.next())
				{
				  
				   String   custNm =rs.getString(2);
				   String  custId=rs.getString(1);
				%> 
				<option value=<%=custId%> ><%=custNm%></option>
					...
					if(request.getParameter("b1")!=null)
					{
						 ...
						 try
							{	
							     ...
							     rs=st.executeQuery("select ordermaster.orderId,  ordermaster.orderDate,  ordermaster.orderCustId, ordermaster.orderCompId, ordermaster.orderAmt, ordermaster.orderGSTAmt ,ordermaster.orderGrandTot  FROM  ordermaster  INNER JOIN  customer ON  ordermaster.orderId = customer.custId and  ordermaster.orderCustId="+request.getParameter("cid"));


							     out.println("<table border='3px;' class='table'>");
									out.println("<tr><th>order id </th><th>order date </th><th> order customer  ID</th><th> Order Company id </th><th>order amount</th><th> order GST amount </th><th>GST Grad total</th></tr>");
							
									while (rs.next()) {
							
										out.println(
										"<tr><th>" + rs.getInt(1) + "</th><th>" + rs.getString(2) +"</th><th>"+rs.getString(3)+"</th><th>"+rs.getString(4)+"</th><th>"+rs.getString(5)+"</th><th>"+rs.getString(6)+"</th><th>"+rs.getString(7)+"</th><th>"+"</th></th></tr>");
